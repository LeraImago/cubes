function main() {
  ...
  function addLineGeometryB(x, y, geometry) {
    const material = new THREE.LineBasicMaterial({color: 0x58C5EA,
      linewidth: 2});
    const mesh = new THREE.LineSegments(geometry, material);
    addObject(x, y, mesh);
  }
   function addLineGeometryC(x, y, geometry) {
    const material = new THREE.LineBasicMaterial({color: 0x49A0FE});
    const mesh = new THREE.LineSegments(geometry, material);
    addObject(x, y, mesh);
  }
    ...
    const material = new THREE.LineBasicMaterial({color: 0x1D3D60});
    const mesh = new THREE.LineSegments(geometry, material);
    addObject(x, y, mesh);
  }
    function addLineGeometryF(x, y, geometry) {
    const material = new THREE.LineBasicMaterial({color: 0x244D7A});
    const mesh = new THREE.LineSegments(geometry, material);
    addObject(x, y, mesh);
  }
  function addLineGeometryG(x, y, geometry) {
    const material = new THREE.LineBasicMaterial({color: 0xFE5058, linewidth: 2});
    const mesh = new THREE.LineSegments(geometry, material);
    addObject(x, y, mesh);
  }
